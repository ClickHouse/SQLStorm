...
StringAggregation AS (
    SELECT 
        c_customer_sk,
        arrayStringConcat(groupArray(assumeNotNull(CONCAT(c_first_name, ' ', c_last_name))), ', ') AS full_name,
        arrayStringConcat(groupArray(assumeNotNull(CONCAT(full_address, ', ', ca_city, ', ', ca_state, ' ', ca_zip))), '; ') AS complete_address,
