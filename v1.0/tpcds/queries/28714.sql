...
WITH AddressStatistics AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(CONCAT(ca_street_number, ' ', ca_street_type)))), ', ') AS unique_address_formats
...
SalesSummary AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(CAST(ws_ship_mode_sk AS VARCHAR)))), ', ') AS ship_modes
    FROM web_sales
