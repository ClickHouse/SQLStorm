...
ClosedPosts AS (
    SELECT 
        ph.PostId,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(cr.Name))), ', ') AS ClosedReasons,
        COUNT(ph.Id) AS CloseCount
    FROM 
        PostHistory ph
    JOIN 
        CloseReasonTypes cr ON CAST(cr.Id AS text) = ph.Comment AND ph.PostHistoryTypeId = 10
    GROUP BY 
        ph.PostId
),
FinalReport AS (
