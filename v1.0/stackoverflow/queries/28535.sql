...
WITH TagCounts AS (
    ...
    FROM Posts
    WHERE PostTypeId = 1 
    GROUP BY arrayJoin(splitByString('><', assumeNotNull(substring(Tags, 2, length(Tags) - 2))))
),
TopTags AS (
    SELECT TagName,
           PostCount,
...
PopularPosts AS (
    ...
    JOIN TagCounts T ON T.TagName = ANY(splitByString('><', assumeNotNull(substring(P.Tags, 2, length(P.Tags) - 2))))
