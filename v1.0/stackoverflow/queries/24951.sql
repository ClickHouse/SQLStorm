...
UserReputation AS (
    SELECT 
        ...
        SUM(CASE WHEN p.CreationDate >= toDateTime64('2024-10-01 12:34:56', 6) - INTERVAL 30 DAY THEN 1 ELSE 0 END) AS RecentPostCount
    FROM 
        Users u
    LEFT JOIN 
...
SELECT 
    ...
    rp.Score,
    rp.ViewCount,
    arrayStringConcat(groupArray(assumeNotNull(ph.Comment)), '; ') FILTER (WHERE ph.Comment IS NOT NULL) AS PostComments,
