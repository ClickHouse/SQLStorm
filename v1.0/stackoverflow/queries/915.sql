...
WITH RankedPosts AS (
    SELECT 
        ...
        p.CreationDate,
        p.Score,
        ROW_NUMBER() OVER (PARTITION BY p.OwnerUserId ORDER BY any(p.LastActivityDate) DESC) AS UserPostRank,
    ...
    WHERE 
        p.Score > 0 AND p.CreationDate >= toDateTime64('2024-10-01 12:34:56', 6) - INTERVAL 1 YEAR
    GROUP BY 
