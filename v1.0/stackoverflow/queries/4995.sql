...
WITH PostEngagement AS (
    SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY any(p.PostTypeId) ORDER BY COALESCE(SUM(CASE WHEN v.VoteTypeId = 2 THEN 1 ELSE 0 END) - SUM(CASE WHEN v.VoteTypeId = 3 THEN 1 ELSE 0 END), 0) DESC) AS Rank
    FROM 
        Posts p
    LEFT JOIN 
