...
SELECT 
    ...
    (SELECT arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(T.TagName))), ', ') FROM Posts P JOIN Tags T ON P.Tags LIKE CONCAT('%', T.TagName, '%') WHERE P.Id = TP.PostId) AS AssociatedTags
FROM 
    TopPosts TP
ORDER BY 
