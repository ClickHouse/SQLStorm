...
WITH UserStats AS (
    SELECT 
        ...
        AVG(COALESCE(NULLIF(toUnixTimestamp((P.LastEditDate - P.CreationDate)), 0), NULL)) AS AvgEditTime,
...
ClosedPosts AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(PH.Comment))), '; ') AS ClosureReasons
    FROM PostHistory PH
