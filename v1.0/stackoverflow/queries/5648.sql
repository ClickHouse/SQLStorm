...
SELECT 
    ...
    (SELECT arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(pt.Name))), ', ') FROM PostTypes pt JOIN Posts p ON p.PostTypeId = pt.Id WHERE p.Id = tp.PostID) as PostType
FROM 
    TopPosts tp
ORDER BY 
