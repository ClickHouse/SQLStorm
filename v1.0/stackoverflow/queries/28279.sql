...
WITH RankedPosts AS (
    SELECT 
        ...
        p.CreationDate,
        toUnixTimestamp((CURRENT_TIMESTAMP - p.CreationDate)) / 60 AS AgeInMinutes,
        SUM(CASE WHEN v.VoteTypeId = 2 THEN 1 ELSE 0 END) AS UpVotes,
        SUM(CASE WHEN v.VoteTypeId = 3 THEN 1 ELSE 0 END) AS DownVotes,
        COUNT(c.Id) AS CommentCount,
        ROW_NUMBER() OVER (PARTITION BY p.Tags ORDER BY toUnixTimestamp((CURRENT_TIMESTAMP - p.CreationDate)) / 60 DESC) AS Rank
    FROM 
        Posts p
    LEFT JOIN 
