WITH SupplierInfo AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(CONCAT('Part: ', p.p_name, ', Comment: ', p.p_comment)))), '; ') AS PartDetails
    FROM 
        supplier s
    JOIN 
...
NationInfo AS (
    SELECT 
        ...
        arrayStringConcat(groupArray(assumeNotNull(si.s_name)), ', ') AS Suppliers
    FROM 
        nation n
    JOIN 
