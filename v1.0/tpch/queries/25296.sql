...
Group_Stats AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(part_name))), ', ') AS part_names,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(comment_preview))), ', ') AS comments
    FROM 
        String_Stats
    GROUP BY 
        name_length
