...
CastInfo AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(CONCAT(a.name, ' (', r.role, ')')))), ', ') AS cast_details
    FROM 
        cast_info AS c
    JOIN 
...
MovieSummary AS (
    SELECT 
        ...
        ct.total_cast,
        ct.cast_details,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(rt.movie_keyword))), ', ') AS keywords
    FROM 
        RankedTitles AS rt
    JOIN 
