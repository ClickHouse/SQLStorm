WITH movie_info_summary AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(ki.keyword))), ', ') AS keyword_list,
...
cast_summary AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(an.name))), ', ') AS cast_list
    FROM cast_info ci
