...
title_info AS (
    SELECT 
        ...
        t.production_year,
        t.kind_id,
        arrayStringConcat(groupArray(assumeNotNull(CONCAT(aka.name, ' (', aka.name_pcode_nf, ')'))), ', ') AS aka_names
    FROM 
        title t
    LEFT JOIN 
...
cast_details AS (
    SELECT 
        c.movie_id,
        arrayStringConcat(groupArray(assumeNotNull(CONCAT(p.name, ' as ', r.role))), ', ') AS cast_info
    FROM 
        cast_info c
    JOIN 
