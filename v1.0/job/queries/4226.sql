...
WITH RankedMovies AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(ka.name))), ', ') AS actors_list,
...
MovieKeywords AS (
    SELECT 
        mk.movie_id,
        arrayStringConcat(groupArray(assumeNotNull(k.keyword)), ', ') AS associated_keywords
    FROM 
        movie_keyword mk
    JOIN 
