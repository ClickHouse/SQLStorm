...
ActorRoles AS (
    SELECT
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(p.name))), ', ') AS actor_names
    FROM
        cast_info ci
    JOIN
...
SELECT
    ...
    m.number_of_actors,
    m.actor_names,
    arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(rt.keyword))), ', ') AS keywords
FROM
    movie_info mi
JOIN
