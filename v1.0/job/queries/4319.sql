...
ActorRoles AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(CONCAT(a.name, ' (', rt.role, ')')))), ', ') AS actor_roles
    FROM 
        cast_info ci
    JOIN 
...
MovieCompanies AS (
    SELECT 
        mc.movie_id,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(cn.name || ' - ' || ct.kind))), '; ') AS companies
    FROM 
        movie_companies mc
    JOIN 
