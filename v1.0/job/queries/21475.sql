...
CompanyDetails AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(cn.name))), ', ') FILTER (WHERE cn.country_code IS NOT NULL) AS company_names
    FROM 
        movie_companies mc
    JOIN 
...
MovieInfo AS (
    SELECT
        mi.movie_id,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(mi.info))), '; ') AS infos
    FROM 
        movie_info mi 
    GROUP BY 
        mi.movie_id
