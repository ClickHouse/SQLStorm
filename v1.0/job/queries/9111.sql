...
cast_details AS (
    SELECT c.movie_id, arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(a.name))), ',') AS actors, COUNT(DISTINCT c.person_id) AS actor_count
    FROM cast_info c
    JOIN aka_name a ON c.person_id = a.person_id
    GROUP BY c.movie_id
),
company_details AS (
    SELECT mc.movie_id, arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(cn.name))), ',') AS companies, ct.kind AS company_type
