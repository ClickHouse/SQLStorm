...
WITH movie_details AS (
    SELECT 
        ...
        mv.production_year,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(actor.name))), ', ') AS cast,
...
company_details AS (
    SELECT 
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(cty.kind))), ', ') AS company_types
    FROM 
        movie_companies mc
    JOIN 
