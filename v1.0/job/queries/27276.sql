...
MovieInfo AS (
    SELECT
        mi.movie_id,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(mi.info))), '; ') AS information
    FROM
        movie_info mi
    GROUP BY
        mi.movie_id
...
CompleteCast AS (
    SELECT
        cc.movie_id,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(p.name))), ', ') AS cast_list
    FROM
        complete_cast cc
    JOIN
