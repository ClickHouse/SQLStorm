WITH movie_details AS (
    SELECT 
        ...
        t.production_year,
        t.kind_id,
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(ak.name))), ', ') AS alternative_titles,
        ...
        arrayStringConcat(arrayDistinct(groupArray(assumeNotNull(pt.role))), ', ') AS cast_roles
    FROM 
        aka_title t
    LEFT JOIN 
